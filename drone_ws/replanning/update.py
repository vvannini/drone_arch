...
#verify if has already an 'end' goal

for step in mission["mission_execution"]:
    # print(step["command"])
    # print(step["instructions"]["area"])

    goal = create_predicate(step["command"], [diagnostic_msgs.msg.KeyValue("region", step["instructions"]["area"])])
    add_goal(goal)
    f.function_value = f.function_value + 1
