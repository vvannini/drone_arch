...
def goto_AG(command):
    while(status != 2):
        setArm()
        setTakeoffMode()
    #wp_file = run_genetic(origin_lat=regions[command[3]][1], origin_long=regions[command[3]][2], origin_alt= regions[command[3]][3], destination_lat= regions[command[4]][1], destination_long=regions[command[4]][2], destination_atl=regions[command[4]][3], map_id=0 )
    #print(str(regions[command[4]][0])+" "+str(regions[command[4]][1])+" "+str(regions[command[4]][2]))
    # regions[command[4]][0], regions[command[4]][1], regions[command[4]][2], 0 ))
#continua essa lista com os seguintes argumentos: origin_lat, origin_long, origin_alt, destination_lat, destination_long, destination_alt, map_id=0
    #print("goto "+wp_file)
