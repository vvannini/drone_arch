...
class Route():
    ...
    def __init__(self, route, mission):
        self.route = route
        self.geo_route = Controller.transform_geo_points(route, mission.area.geo_home) #ToDo: checar essa funcao
...
class Controller():
    ...
    def calc_route(self): #throws FileNotFoundException, IOException
        ...
        print("Precisao em milimetros: " + str(Controller.calc_GSD(mission)))
        print("-------------------------------------------------------------------------------")

        routes = [] #List of CartesianPoints

        if mission.drone.min_battery < route.flight_duration:
            mission_qty = math.ceil((route.flight_duration / 60) / mission.drone.min_battery) # ToDo: verificar essa funcao da math

            index = Controller.split_index(route.route, mission_qty) # ToDo: verificar essa funcao
            ...
            for m in range(mission_qty):
                ...
                print("----------------------------------------------(SubRoute ", m, ")---------------------------------------------------")
                print("Numero de WayPoints: ", len(sub_route))
                print("Comprimento da rota: ", sub.route_length)
                print("Numero de Fotos ", sub.picture_qty)
                print("Flight Time ", sub.flight_duration / 60)
                print("Pictures Per Second ", sub.picture_per_second)
                #print(sub.geoRoute.stream().map(e -> e.toString()).reduce(String::concat).get()) # ToDo: verificar funcao
            ...
            path_return = Controller.save_litchi(routes, mission)
    ...
    '''
    /**
     * Faz a criacao do arquivo KML para poder fazer a visualizacao da rota
    ...
    def calc_heading_cartesian(c1, c2):
        ...
        heading = heading % 360 #if heading>360 else heading # ToDo: simplificar com expressao de cima
