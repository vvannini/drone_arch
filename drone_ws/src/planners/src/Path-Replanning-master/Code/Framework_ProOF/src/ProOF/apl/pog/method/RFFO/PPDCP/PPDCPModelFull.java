 ...
 * @author marcio
 */
public class PPDCPModelFull extends CplexFull{
    ...
    public PPDCPModelFull() throws IloException {
    ...
    public void model() throws Exception {
        ...
        Zjti = new IloIntVar[inst.J][inst.T+1][];
        for(int j=0; j<inst.J; j++){
            for(int t=0; t<inst.T+1; t++){
                Zjti[j][t] = cpx.boolVarArray(inst.G(j), "Z"+(j+1)+""+(t+1));
            }
        }
...
//                        System.out.printf("minU: %12g\n", minU);
                        ...
                        inst.plot(String.format("%s - %6.2f %%", "model-full", gap), vMt, vDt, getObjValue(), total_dist, gap, time_best);
                    }
                }
            }
        });
        if(PPDCPInstance.PLOT){
            cpx.use(new IloCplex.MIPInfoCallback() {
                ...
                protected void main() throws IloException {
                    ...
                    inst.title(String.format("%s - %6.2f %%", "model-full", gap));
                }
            });
        }
    }
