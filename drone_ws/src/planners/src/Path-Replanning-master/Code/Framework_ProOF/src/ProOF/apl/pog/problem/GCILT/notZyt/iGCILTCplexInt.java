...
public final class iGCILTCplexInt extends aGCILTModel{
    ...
    private IloNumVar Hulyt[][][][];
    private IloNumVar Glyt[][][];
    ...
    public void start() throws Exception {
        ...
        Zyt = cplex.boolVarArray(gcilt.inst.Y, gcilt.inst.T, "Z");
        
        Hulyt = cplex.numVarArray(gcilt.inst.L, gcilt.inst.L, gcilt.inst.Y, gcilt.inst.T, 0, Double.MAX_VALUE, "H"); 
        ...
        for(int u=0; u<gcilt.inst.L; u++){
            for(int l=0; l<gcilt.inst.L; l++){
                for(int t=0; t<gcilt.inst.T; t++){
                    for(int y=0; y<gcilt.inst.Y; y++){
                        cplex.addLe(Hulyt[u][l][y][t], Zyt[y][t], "Subject03a("+u+","+l+","+y+","+t+")");
                    }
                }
            }
        }
        ...
        for(int l=0; l<gcilt.inst.L; l++){
            for(int t=0; t<gcilt.inst.T; t++){
                for(int y=0; y<gcilt.inst.Y; y++){
                    for(int k : gcilt.inst.Ky[y]){
                        IloNumExpr sum = cplex.prod(Glyt[l][y][t], 1);
                        for(int i : gcilt.inst.Nl[l]){
                            sum = cplex.sum(sum, cplex.prod(Xikt[i][k][t], 1));
                        }
        ...
        for(int l=0; l<gcilt.inst.L; l++){
           for(int t=0; t<gcilt.inst.T; t++){
               for(int y=0; y<gcilt.inst.Y; y++){
                   cplex.addLe(Glyt[l][y][t], cplex.prod(Zyt[y][t],gcilt.inst.Qt[t]),
                            "Subject04a("+l+","+y+","+t+")");
               }
           }
        }
