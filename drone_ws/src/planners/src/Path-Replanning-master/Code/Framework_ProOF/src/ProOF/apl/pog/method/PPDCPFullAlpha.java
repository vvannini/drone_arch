 ...
 * @author Hossomi
 */
public class PPDCPFullAlpha extends Run {
    ...
    public void start() throws Exception {
        ...
        Bjti = new IloNumVar[inst.J][inst.T][];
        for(int j=0; j<inst.J; j++){
            for(int t=0; t<inst.T; t++){
                Bjti[j][t] = cplex.numVarArray(inst.bji[j].length, 0, Integer.MAX_VALUE, "B"+(j+1)+""+(t+1));
            }
        }
            ...
            for(int t=0; t<inst.T; t++){
                for(int i=0; i<inst.bji[j].length; i++){
                    if(t==0){
                        ...
                        cplex.addGe(Bjti[j][t][i], cplex.sum(cplex.prod(Zjti[j][t][i], +1),cplex.prod(Zjti[j][t-1][i], -1)), "B1."+(j+1)+","+(t+1));
                        cplex.addGe(Bjti[j][t][i], cplex.sum(cplex.prod(Zjti[j][t][i], -1),cplex.prod(Zjti[j][t-1][i], +1)), "B2."+(j+1)+","+(t+1));
                    }

                    //cplex.addGe(Cjtk[j][t][i], cplex.sum(cplex.prod(Zjti[j][t][i], +1),cplex.prod(Zjti[j][t-1][i], -1)), "B1."+(j+1)+","+(t+1));
                    //cplex.addGe(Cjtk[j][t][i], cplex.sum(cplex.prod(Zjti[j][t][i], -1),cplex.prod(Zjti[j][t-1][i], +1)), "B2."+(j+1)+","+(t+1));
    ...
    public void execute() throws Exception {
        ...
        //cplex.exportModel("PPDCPFullAlpha.lp");
