 ...
 * @author marcio
 */
public class MultiBinRealFunction extends MultiFunction {
    
    public MultiBinRealFunction(Factory realMulti, Factory realOperator) {
        super(realMulti, realOperator);
    }
    
    @Override
    public String name() {
        return "Multi - BinReal";
    }
    ...
    private class Obj extends MultiObjective<Problem, cBinaryReal, MultiObjective> {
        public Obj(BoundDbl... bounds) throws Exception {
            super(bounds);
        }
        @Override
        public void evaluate(Problem prob, cBinaryReal codif) throws Exception {
            double x[] = codif.X();
            double penality = func.constrant(x);
            for(int i=0; i<func.goals(); i++){
                set(i, func.evaluate(i, x) + penality);
            }
        }
        @Override
