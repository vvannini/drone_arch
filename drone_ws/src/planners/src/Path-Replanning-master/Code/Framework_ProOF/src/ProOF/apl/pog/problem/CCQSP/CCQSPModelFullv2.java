 ...
 * @author marcio
 */
public final class CCQSPModelFullv2 {
    ...
    public static void main(String args[]) throws IOException, IloException{
        ...
        CCQSPModelFullv2 model = new CCQSPModelFullv2(cplex, inst, "CCQSP");
        model.execute(10000, 0, 1);
        model.plot();
    }
    
    public CCQSPModelFullv2(CplexExtended cplex, CCQSPInstance inst, String name) throws IloException {
        this.cplex = cplex;
        this.inst = inst;
        this.name = name;
...
//            Xt[t][2].setUB(inst.map.maxY);
        ...
        Tk = cplex.intVarArray(inst.I(), 0, inst.map.T, "T");
        
        
        for(int k=0; k<inst.I(); k++){
            if(k==0){
                cplex.addGe(Tk[k], inst.qsp.LBk[k]/inst.map.DT, "Tk_LB");
                cplex.addLe(Tk[k], inst.qsp.UBk[k]/inst.map.DT, "Tk_UB");
            }else{
                IloNumExpr exp = cplex.sum(Tk[k], cplex.prod(-1, Tk[k-1]));
                cplex.addGe(exp, inst.qsp.LBk[k]/inst.map.DT, "Tk_LB");
                cplex.addLe(exp, inst.qsp.UBk[k]/inst.map.DT, "Tk_UB");
            }
        }
        ...
        for(int j=0; j<inst.I(); j++){
            for(int t=0; t<inst.T()+1; t++){
//                IloNumExpr expLB = cplex.sum(cplex.prod(-1, Tk[j]),   +(t)*inst.map.DT);    //-Tk<=0
//                IloNumExpr expUB = cplex.sum(Tk[j],   -(t+1)*inst.map.DT);             //Tk<=1.9999
//                System.out.printf("j%d | t%d | LB = %s and UB = %s\n", j, t, expLB, expUB);
//                cplex.addIF_Le_Them_Y("I", Ijt[j][t], expLB, expUB);
                
                IloNumExpr exp = cplex.sum(Tk[j],   -t);
        ...
        //IloNumExpr obj = cplex.prod(10, Tk[inst.qsp.K-1]);
        IloNumExpr obj = null;
...
//    }
    ...
    protected void ObstacleAvoidanceFree(CplexExtended cplex, IloNumVar[][] MUt, IloIntVar[][][] Zjti, IloNumVar Djt[][], final int DN) throws IloException{
        ...
        for(int j=0; j<inst.map.J; j++){
            for(int t=0; t<inst.map.T+1; t++){
                for(int i=0; i<inst.G(j); i++){
                    IloNumExpr exp = cplex.prod(-inst.map.Ob[j].lines[i].ax, MUt[t][0]);
                    exp = cplex.sum(exp, cplex.prod(-inst.map.Ob[j].lines[i].ay, MUt[t][2]));
                    exp = cplex.sum(exp, cplex.prod(inst.map.Mjti[j][t][i], Zjti[j][t][i]));
                    double val = inst.map.Mjti[j][t][i] - inst.map.Ob[j].lines[i].b;
                    
                    exp = cplex.SumProd(exp, 1, cplex.RiskAllocation(Djt[j][t], inst.map.Rjti[j][t][i], inst.map.DELTA, DN, "RiskAlloc"));
    ...
    /*
    protected void ObstacleAvoidanceFree(ProOFCplex cplex, IloNumVar[][] MUt, IloIntVar[][][] Zjti, IloNumVar Djt[][], final int DN) throws IloException{
    ...
    */
    ...
    private double M(int n){
        ...
        double y0 = Normal.inverseStandardCdf((1+x0)/2.0)/Math.sqrt(2); //Normal.inverseStandardCdf(x0);
        double y1 = Normal.inverseStandardCdf((1+x1)/2.0)/Math.sqrt(2); //Normal.inverseStandardCdf(x1);
