 ...
 * @author marcio
 */
public final class CCQSPModelFullvReal {
    ...
    public static void main(String args[]) throws IOException, IloException{
        ...
        CCQSPModelFullvReal model = new CCQSPModelFullvReal(cplex, inst, "CCQSP");
        model.execute(10000, 0, 1);
        model.plot();
    }
    
    public CCQSPModelFullvReal(CplexExtended cplex, CCQSPInstance inst, String name) throws IloException {
        this.cplex = cplex;
        this.inst = inst;
        this.name = name;
...
//            Xt[t][2].setUB(inst.map.maxY);
        ...
        Tk = cplex.numVarArray(inst.I(), 0, inst.map.T*inst.map.DT+2*epsilon, "T");
        
        
        for(int k=0; k<inst.I(); k++){
            if(k==0){
                cplex.addGe(Tk[k], inst.qsp.LBk[k], "Tk_LB");
                cplex.addLe(Tk[k], inst.qsp.UBk[k], "Tk_UB");
            }else{
                IloNumExpr exp = cplex.sum(Tk[k], cplex.prod(-1, Tk[k-1]));
                cplex.addGe(exp, inst.qsp.LBk[k], "Tk_LB");
                cplex.addLe(exp, inst.qsp.UBk[k], "Tk_UB");
            }
        }
...
//                cplex.addIF_Le_Them_Y("I", Ijt[j][t], expLB, expUB);
                IloNumExpr expLB = cplex.sum(Tk[j],   -(t)*inst.map.DT);
                IloNumExpr expUB = cplex.sum(Tk[j],   -(t+1)*inst.map.DT-epsilon);
                IloIntVar z = cplex.boolVar("z");
                IloIntVar w = cplex.boolVar("w");
                cplex.addIF_Le_Them_Y("I.Le", z, expUB);
                cplex.addIF_Ge_Them_Y("I.Ge", w, expLB);
                cplex.And("I.and", Ijt[j][t], z, w);
