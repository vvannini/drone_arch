 ...
 * @author marcio
 */
public class Norm2SQR_Cpx implements AddObjective {
    private final PPDCPInstance inst;

    public Norm2SQR_Cpx(PPDCPInstance inst) {
        this.inst = inst;
    }
    ...
    public IloNumExpr objective(CplexExtended cplex, IloNumVar[][] Ut, IloNumVar[][] MUt) throws Exception {
        ...
        for (int t = 0; t < inst.T; t++) {
            if (obj == null) {
                obj = cplex.sum(cplex.prod(inst.P_Obj, cplex.prod(Vt[t][0], Vt[t][0])), cplex.prod(inst.P_Obj, cplex.prod(Vt[t][1], Vt[t][1])));
            } else {
                obj = cplex.sum(obj, cplex.prod(inst.P_Obj, cplex.prod(Vt[t][0], Vt[t][0])), cplex.prod(inst.P_Obj, cplex.prod(Vt[t][1], Vt[t][1])));
            }
        }
        return obj;
    }
