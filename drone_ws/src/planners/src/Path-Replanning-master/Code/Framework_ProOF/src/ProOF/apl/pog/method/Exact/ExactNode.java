 ...
 * @param <D>
 */
public abstract class ExactNode <Prob extends aComb, N extends ExactNode, D> implements Comparable<ExactNode>{
    ...
    public ExactNode(Prob prob, N back, D data) throws Exception {
    ...
    @Override
    public int compareTo(ExactNode o) {
    ...
    public void results(Prob prob, LinkerResults link, ExactNode base) throws Exception {
        link.writeLong("id", id);
        link.writeInt("level", level);
        link.writeDbl("cost", cur_cost);
        link.writeString("sol", toString());
    }
    public void printer(Prob prob, StreamPrinter link, ExactNode base) throws Exception{
